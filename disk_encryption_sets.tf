module "disk_encryption_sets" {
  ...
  key_vault_key_id    = try(each.value.auto_key_rotation_enabled, false) == true ? coalesce(try(each.value.key_vault_key_id,null), try(each.value.key_vault_key.id, null),try(local.combined_objects_keyvault_keys[try(each.value.keyvault_key.lz_key, local.client_config.landingzone_key)][try(each.value.key_vault_key_key, each.value.key_vault_key.key)].versionless_id, null)) : coalesce(try(each.value.key_vault_key_id,null), try(each.value.key_vault_key.id, null),try(local.combined_objects_keyvault_keys[try(each.value.keyvault_key.lz_key, local.client_config.landingzone_key)][try(each.value.key_vault_key_key, each.value.key_vault_key.key)].id, null))

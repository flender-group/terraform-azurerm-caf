resource "azuread_app_role_assignment" "app" {
  ...
  principal_object_id = coalesce(
    ...
    try(var.azuread_dynamic_groups[try(var.settings.azuread_dynamic_group.lz_key, var.client_config.landingzone_key)][var.settings.azuread_dynamic_group.key].object_id, null),
    try(var.settings.object_id, null)
  )
}
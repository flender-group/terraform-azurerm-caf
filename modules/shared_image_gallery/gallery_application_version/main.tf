locals {
  ...
  #installer_sas_url         = can(var.settings.media_link) ? null : "${local.installer_storage_account.primary_blob_endpoint}${local.installer_storage_account.containers[var.settings.storage_accounts.container_key].name}${data.azurerm_storage_account_sas.installer[0].sas}"

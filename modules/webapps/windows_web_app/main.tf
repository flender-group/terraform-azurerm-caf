...
locals {
  ...
  http_logs_storage_account = can(var.settings.logs.http_logs.azure_blob_storage) ? var.storage_accounts[try(var.settings.logs.http_logs.azure_blob_storage.lz_key, var.client_config.landingzone_key)][var.settings.logs.http_logs.azure_blob_storage.storage_account_key] : null
  http_logs_sas_url         = can(var.settings.logs.http_logs.azure_blob_storage) ? "${local.http_logs_storage_account.primary_blob_endpoint}${local.http_logs_storage_account.containers[var.settings.logs.http_logs.container_key].name}${data.azurerm_storage_account_blob_container_sas.http_logs[0].sas}" : null
}
